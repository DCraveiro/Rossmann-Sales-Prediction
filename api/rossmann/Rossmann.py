...
class Rossmann(object):
    def __init__(self):
        # home path
        self.home_path = 'C:/Users/debor/repos/DS-Community-Course/'
    ...
    def feat_engineering(self, df2):
        ...
        df2['competition_time_month'] = ((df2['date'] - df2['competition_since']) / 30).apply(lambda x: x.days).astype(
            int)  # how long there is a competitor nearby in months

        # promo since
        df2['promo_since'] = df2['promo2_since_year'].astype(str) + '-' + df2['promo2_since_week'].astype(str)
        df2['promo_since'] = df2['promo_since'].apply(
            lambda x: datetime.datetime.strptime(x + '-1', '%Y-%W-%w') - datetime.timedelta(days=7))
        df2['promo_time_week'] = ((df2['date'] - df2['promo_since']) / 7).apply(lambda x: x.days).astype(
            int)  # how long a promotion is active in weeks
    ...
    def data_preparation(self, df5):
        ...
        cols_selected = ['store', 'promo', 'store_type', 'assortment', 'competition_distance',
                         ...
                         'promo_time_week', 'day_of_week_sin', 'day_of_week_cos', 'month_cos', 'month_sin', 'day_sin',
                         'day_cos',
